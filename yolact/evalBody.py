...
"""
    ...
    if args.display_masks and cfg.eval_mask_branch and num_dets_to_consider > 0:
        ...
        body = img_numpy
        H = "results/SUS.png"#CHANGE
        head = Image.open(H)
        
        x1, y1, x2, y2 = boxes[j, :]
        bwidth = np.abs(x2 - x1)
        bheight = np.abs(y2 - y1)
        scale_factor = max(bwidth/head.width,bheight/head.height)
        Bcenter_x = (x1+x2)//2
        Bcenter_y = (y1+y2)//2
        ...
        head_numpy = cv2.cvtColor(head_numpy, cv2.COLOR_RGB2BGR)
        ...
        img_numpy = body_numpy
        # cv2.imwrite("tmp.png",img_numpy)
        #res = Image.open("tmp.png")
        #res.show()
