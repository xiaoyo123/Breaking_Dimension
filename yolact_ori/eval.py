...
def prep_display(dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=0.45, fps_str=''):
    ...
    person_indices = [j for j in range(num_dets_to_consider) if cfg.dataset.class_names[classes[j]] == "person"]
    
    num_dets_to_consider = len(person_indices)
    ...
    if args.display_text or args.display_bboxes:
        for i, j in enumerate(person_indices):
            x1, y1, x2, y2 = boxes[j, :]
            color = get_color(j)
            score = scores[j]
            ...
            if args.display_bboxes:
                midx=(x1+x2)/2
                img_numpy = img_numpy[y1:y2, x1:x2]
                #img_numpy = img_numpy
